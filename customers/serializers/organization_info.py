        ...
        if is_private:
            if not first_name:
                raise ServiceErrorHandler(
                    "First name is required for private organizations"
                )
            if not last_name:
                raise ServiceErrorHandler(
                    "Last name is required for private organizations"
                )
            attrs["organization_name"] = f"{first_name} {last_name}".strip()
        else:
            # Modified validation for non-private organizations
